...
export const api = {
  ...
  // Plans & Payments
  async getPlans(): Promise<{ plans: Record<string, unknown> }> {
    const { data } = await client.get('/api/plans')
    return data
  },

  async preparePayment(plan: string): Promise<{ merchant_uid: string; amount: number }> {
    const { data } = await client.post('/api/payment/prepare', { plan })
    return data
  },

  async verifyPayment(impUid: string, merchantUid: string): Promise<{ success: boolean; plan: string; expires_at: string }> {
    const { data } = await client.post('/api/payment/verify', { imp_uid: impUid, merchant_uid: merchantUid })
    return data
  },

  async getPaymentHistory(): Promise<{ payments: unknown[] }> {
    const { data } = await client.get('/api/payment/history')
    return data
  },
}
