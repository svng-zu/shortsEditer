...
score(중요도) 책정 기준 — 매우 중요:
- 골/득점 장면, 결승골, 역전골 등 스코어가 바뀌는 장면은 9~10점을 부여하세요.
- 결정적인 명장면(슈퍼세이브, 결정적 수비, 극적인 역전 직전 상황)은 7~8점을 부여하세요.
- 팀 소개, 경기 배경 설명, 일반 해설 구간은 4~6점을 부여하세요.
- 전체 길이가 길어 일부 구간만 남길 경우, score가 높은 구간(특히 골 장면)이
  최우선으로 살아남으므로 골/득점 장면일수록 반드시 더 높은 score를 매기세요.
"""
}
...
조건:
- 첫 문장은 전체 내용을 한 줄로 요약하는 후킹 멘트로 시작. "아니", "와" 같은 감탄사로 시작해
  호기심을 자극하는 반응형 말투로 핵심 내용을 압축해서 던지고("~다는데?", "~했다는 거 아니야?" 등),
  이어지는 문장부터 본 내용을 설명. 예: "아니 ~당대표가 ~을 했다는데?"
...
조건:
- 가장 첫 문장으로, 전체 내용을 한 줄로 요약하는 후킹 멘트를 새로 추가하세요. "아니", "와" 같은
  감탄사로 시작해 호기심을 자극하는 반응형 말투로 핵심 내용을 압축해서 던지고("~다는데?",
  "~했다는 거 아니야?" 등), 그다음 문장부터 원본 내용을 설명. 예: "아니 ~당대표가 ~을 했다는데?"
- 이 후킹 멘트 이후부터는 절대 요약하지 말고, 원본에 담긴 정보·수치·순서를 빠짐없이 전부 유지하세요
- "~입니다/~했습니다" 같은 뉴스 앵커식 격식체를 "~하더라고요/~한대요/~거든요/~인데요" 같은
  친근한 구어체로 바꾸세요
- 후킹 멘트를 제외하고는 새로운 정보를 추가하거나 내용을 생략하지 말고, 표현과 어조만 바꾸세요
- 문장이 자연스럽게 끝나도록 마침표로 종결
