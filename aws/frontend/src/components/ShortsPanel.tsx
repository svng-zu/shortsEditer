...
// 메인
export default function ShortsPanel({ activeTab, onTabChange, raws, shorts, selectedRaw, selectedShort, onSelectRaw, onSelectShort, onRefresh, onStartPolling, isRunning }: Props) {
        ...
        {activeTab === 'raws' && (
          <>
            ...
            <div style={{ width: 220, flexShrink: 0, borderRight: '1px solid var(--border)', overflowY: 'auto', background: 'var(--surface2)' }}>
              ...
              {raws.length === 0
                ? <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--muted)' }}><div style={{ fontSize: 24, marginBottom: 6, opacity: 0.4 }}>🎞</div><p style={{ fontSize: 11 }}>영상 편집 후 표시됩니다</p></div>
                : raws.map(r => (
                    <div key={r.filename} onClick={() => onSelectRaw(r)} style={{
                      ...
                      <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', lineHeight: 1.4, marginBottom: 4 }}>{r.title || r.filename}</div>
            ...
            {/* 편집 영역 */}
            <RawEditArea raw={selectedRaw} onStartPolling={onStartPolling} isRunning={isRunning} />
          </>
        )}
        ...
        {activeTab === 'shorts' && (
          <>
            ...
            <div style={{ width: 220, flexShrink: 0, borderRight: '1px solid var(--border)', overflowY: 'auto', background: 'var(--surface2)' }}>
              ...
              {shorts.length === 0
                ? <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--muted)' }}><div style={{ fontSize: 24, marginBottom: 6, opacity: 0.4 }}>🎬</div><p style={{ fontSize: 11 }}>렌더링 후 표시됩니다</p></div>
                : shorts.map(s => (
                    <div key={s.filename} onClick={() => onSelectShort(s)} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', lineHeight: 1.4, marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.title || s.filename}</div>
...
// Raw 편집 영역
function RawEditArea({ raw, onStartPolling, isRunning }: { raw: RawInfo | null; onStartPolling: () => void; isRunning: boolean }) {
  ...
  useEffect(() => {
    ...
    vid.src = raw.url; vid.muted = true; vid.loop = true; vid.play().catch(() => {})
  ...
  }, [raw?.filename, title1, title2, t1Color, t2Color, titleY, titleScale, subtitles, subSize, subColor, subY, bgImage, templateId])
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', background: 'var(--surface)' }}>
      <video ref={hidVidRef} style={{ display: 'none' }} muted loop playsInline />

      {/* ── 왼쪽: 큰 캔버스 미리보기 ── */}
      <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--surface2)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
          {raw ? (
            <>
              {/* 캔버스 — CSS로 컨테이너 너비에 맞게 확대 표시 */}
              <div style={{ width: '100%', borderRadius: 10, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.12)', border: '1px solid var(--border)' }}>
              ...
              <details style={{ width: '100%' }} open={false}>
                <summary style={{ fontSize: 11, color: 'var(--text2)', cursor: 'pointer', padding: '4px 2px', userSelect: 'none' }}>
                  ▸ 원본 영상
                </summary>
        ...
        {!raw
          ...
          : <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 480 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={handleRender} disabled={isRendering || isRunning} className="btn-primary"
